...
# Add parent directory to sys.path (which is backend)
...
async def reproduce():
    ...
    # 3. Verify Relevance
    logger.info("Starting relevance verification...")
    ...
    if not verified_docs:
        logger.warning("FAILED: No documents passed verification.")
    else:
        logger.info("SUCCESS: One or more documents passed verification.")
        
    for doc in all_docs:
        status = "PASSED" if doc in verified_docs else "REJECTED"
        logger.info(f"Doc {doc.id} ({doc.collection}): {status}")
        # Use debug to show content
        logger.info(f"Content snippet: {doc.content[:150]}...")
